...
- `require_user` API-key dependency for protected endpoints.
...
- QA harness with trustworthy pass-rate, soft thresholds, and refusal-aware
  canaries.
- Empty-graph in-shape sentinel for `search_relationships`, `find_experts`,
  and `trace_decision_history`.
- Output-token-aware adaptive batching in the ingestion pipeline.
- LLM EOF hardening and QA chat UI refactor.
- File import pipeline and global Ask/QA session.
- Pluggable graph database layer with Neo4j and NebulaGraph backends
  (`GRAPH_BACKEND` env var).
- Wiki generation, channel wiki view, FAQ page, and Mermaid edge labels.
...
- Slack adapter: raw Slack API for message fetch and file proxy auth;
  `node-emoji` for reactions; cleaner mrkdwn parsing; thread replies
  fetched; thread-aware batching with refined bot filtering.
...
- Mermaid v11 silent syntax-error SVGs caught via pre-validate and content
  sniffing.
- Wiki and Ask language/session fallbacks.
- QA skills: L3 resource files wired into ADK Resources and registered
  under `assets`; QA tools exposed as siblings of `SkillToolset`.
- Follow-up suggestions: bullet-strip guard and prompt contract.
- Slack: channel sync errors and connection handling; attachment images
  and unfurls; raw files and attachment unfurls for media display.
- Teams, Telegram, Discord adapter configs aligned with Chat SDK docs.
